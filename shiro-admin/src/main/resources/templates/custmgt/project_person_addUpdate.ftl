...
<script>
    var person_options = {
        url: "/custmgt/person/personRelationListByProjectId",
    ...
    var person_url;
    function addUpdatePersonTable(projectId,operation) {
        if(operation!='view'){
        ...
        addPersonList = [];
        person_url=person_options.url + "?projectId=" + projectId;
    ...
    $("#btn_add_person").click(function () {
        ...
        $('#choose_list').modal();
        //从url加载数据到模态框
        $('#choose_list').load('/custmgt/person/chooseViewByProject');
    });
    ...
    $("#btn_delete_person_ids").click(function () {
        ...
        for(var i = 0; i < selecteded.length; i++) {
            var s=selecteded[i];
            if(s.currProjectId==null||s.currProjectId==''){
                rowcounts.push(s.id);
                //移除临时记录
    ...
    function deletePersonRelationIds(ids) {
        $.ajax({
            ...
            async: false,
            traditional: true,
            url: "/custmgt/project/deletePersonRelationByProjectId?projectId="+$("#addOrUpdateForm #id").val(),
            data: {'personIds': ids},
